...
SELECT CONCAT(Name, '-', Quantity, '-', OrderStatus)  FROM item,mg_orders WHERE item.ItemID = mg_orders.ItemID; 
SELECT CONCAT(LOWER(Name), '-', Quantity, '-', UPPER(OrderStatus)) AS 'ITEM STATUS'  FROM item,mg_orders WHERE item.ItemID = mg_orders.ItemID; 

-- Task 2: Write a SQL SELECT query using an appropriate date function and a format string to find 
-- the name of the weekday on which M&G’s orders are to be delivered.
 SELECT DATE_FORMAT(DeliveryDate, "%W") AS 'DELIVERY DAY' FROM mg_orders;
 ...
 SELECT ItemID, ROUND(cost/100*5, 2) AS 'Handling Cost' FROM mg_orders;
 
 -- Task 4: Review the query that you wrote in the second task. Use an appropriate comparison function to filter out 
 -- the records that do not have a NULL value in the delivery date column.
  SELECT DATE_FORMAT(DeliveryDate, "%W") AS 'DELIVERY DAY' FROM mg_orders WHERE !ISNULL(DeliveryDate);  -- or
  SELECT DATE_FORMAT(DeliveryDate, "%W") AS 'DELIVERY DAY' FROM mg_orders WHERE DeliveryDate IS NOT NULL;
...
SELECT ClientID,orderID, Cost-(Cost/100*5) AS 'After Discount'  FROM client_orders WHERE ItemID=4;
  
  -- Task 2: Format the afterDiscount column value from the earlier result for
  -- 5% discount in '#,###,###.##' format rounded to 2 decimal places using the FORMAT function.
SELECT ClientID,orderID, ROUND(Cost-(Cost/100*5),2) AS 'After Discount'  FROM client_orders WHERE ItemID=4;  -- OR
SELECT ClientID,orderID, FORMAT(Cost-(Cost/100*5),2) AS 'After Discount'  FROM client_orders WHERE ItemID=4;
